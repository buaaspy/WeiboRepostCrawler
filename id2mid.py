...
class WeiboExtract:
    ...
    def __init__(self):
        self.data_root_path = "./dat/"
        self.out_root_path = "./cleanroom/gamma/"
    ...
    def extract_one_file(self, file):
        filename = file
        print("[extractor] extract %s" % file)
        ...
        try:
            ...
            for item in soup.find_all(attrs={"action-type" : "feed_list_item"}):
                try:
                    ...
                    print("%s %s %s %s %s" % (filename, mid, name, uid, usercard))
                    retweet = mid + " " + name + " " + uid
